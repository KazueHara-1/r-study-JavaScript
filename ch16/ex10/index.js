...
function serve(rootDirectory, port) {
  const server = new Server(); // 新しいHTTPサーバを作成する。
  ...
  server.on("request", (request, response) => {
    ...
    // クエリパラメータは無視する。
    const endpoint = parse(request.url).pathname;
    ...
    // ファイルアップロード処理 (ex10追加部分)
    else if (request.method === "PUT") {
      const writeStream = createWriteStream(`./uploaded${endpoint}`);
